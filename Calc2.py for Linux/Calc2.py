...
def _register_hindi_font() -> bool:
    ...
    if any("Noto Sans Devanagari" in f for f in families):
        return True
    fid = QFontDatabase.addApplicationFont(":/fonts/NotoSansDevanagari-Regular.ttf")
    return fid >= 0

_LANG_FONTS = {
    "hi": ("Noto Sans Devanagari", 10),
}
...
def _ensure_hindi_font(app) -> None:
    ...
    if msg.exec_() != QMessageBox.Ok:
        return
    ok = _install_font_if_needed(str(font_file), "Noto Sans Devanagari")
    if ok:
...
class CalcFormelHelper(QMainWindow):
    ...
    def __init__(self):
        ...
        _shortcut_hint = QPushButton("  Ctrl+F12: Minimize / Restore  ⓘ")
        _shortcut_hint.setStyleSheet(
            "QPushButton { color: #888; font-size: 9pt; border: none;"
            " background: transparent; padding: 0 4px; }"
            "QPushButton:hover { color: #bbb; text-decoration: underline; }"
        )
        _shortcut_hint.setCursor(Qt.PointingHandCursor)
        _shortcut_hint.setFlat(True)
        _shortcut_hint.clicked.connect(self._show_hotkey_info)
    ...
    def _toggle_minimize(self):
        ...
        # Debounce: Doppelauslösung innerhalb 400 ms ignorieren
        now = time.time()
        ...
        # Den eigentlichen Fenster-Code IMMER im Qt-Hauptthread ausführen.
        # Auf Linux kommt das Signal von evdev aus einem Hintergrund-Thread –
        # dort liefert isMinimized() falsche Werte und GUI-Aufrufe sind
        # nicht thread-sicher. QTimer.singleShot(0) stellt das sicher.
        QTimer.singleShot(0, self._do_toggle_minimize)

    def _do_toggle_minimize(self):
        """Wird immer im Qt-Hauptthread ausgeführt (via QTimer)."""
        from PyQt5 import QtCore
        from PyQt5.QtCore import QTimer
        ...
        if self.isMinimized() or not self.isVisible():
            ...
            self.raise_()
            self.activateWindow()
            # Linux: zweiter activateWindow nach kurzem Delay
            # damit X11/Wayland den Fokus wirklich übergibt
    ...
    def _show_hotkey_info(self):
        """Info button next to Ctrl+F12 hint in the status bar."""
        import platform as _plat
        import grp
        import pwd
        ...
        if _plat.system() != "Linux":
            ...
            QMessageBox.information(
                self, "Ctrl+F12 – Minimize / Restore",
                "Press Ctrl+F12 to minimize or restore this window from anywhere, "
                "even when another application is in the foreground."
            )
            return
        ...
        try:
            ...
            already_in_group = input_gid in user_groups
        except KeyError:
            already_in_group = True   # group doesn't exist → no action needed

        if already_in_group:
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.information(
                self, "Ctrl+F12 – Minimize / Restore",
                "Ctrl+F12 is fully set up on this system.\n\n"
                "Press Ctrl+F12 to minimize or restore this window from anywhere, "
                "even when another application is in the foreground.\n\n"
                "Backend: pynput  with evdev (Wayland) / Xlib (X11)"
            )
            return

        # User is NOT in 'input' group → show explanation and offer fix
        from PyQt5.QtWidgets import QMessageBox
        username = pwd.getpwuid(os.getuid()).pw_name

        msg = QMessageBox(self)
        msg.setWindowTitle("Ctrl+F12 – Administrator Permission Required")
        msg.setIcon(QMessageBox.Information)
        msg.setText("<b>One-time setup required for the global hotkey</b>")
        msg.setInformativeText(
            f"To capture <b>Ctrl+F12</b> system-wide (even when another window "
            f"is focused), this application needs access to keyboard input devices "
            f"(<tt>/dev/input/</tt>).<br><br>"
            f"This requires adding your user account (<b>{username}</b>) "
            f"to the <b>input</b> group — a one-time operation that needs "
            f"administrator (sudo) privileges.<br><br>"
            f"Command that will be executed:<br>"
            f"<tt>sudo usermod -aG input {username}</tt><br><br>"
            f"You will need to <b>log out and log back in</b> afterwards "
            f"for the change to take effect.<br><br>"
            f"Click <b>OK</b> to enter your password and apply the change, "
            f"or <b>Cancel</b> to skip."
        )
        msg.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
        msg.button(QMessageBox.Ok).setText("OK – Apply")
        msg.button(QMessageBox.Cancel).setText("Cancel")

        if msg.exec_() != QMessageBox.Ok:
            return

        self._add_user_to_input_group(username)

    def _add_user_to_input_group(self, username):
        """Runs: pkexec usermod -aG input <username>"""
        import shutil
        from PyQt5.QtWidgets import QMessageBox
        ...
        auth   = pkexec if pkexec else sudo
        cmd    = [auth, "usermod", "-aG", "input", username]
        ...
        try:
            ...
            if result.returncode == 0:
                QMessageBox.information(
                    self, "Done",
                    f"User '{username}' has been added to the 'input' group.\n\n"
                    "Please log out and log back in for the change to take effect.\n"
                    "After that, Ctrl+F12 will work globally."
                )
            else:
                ...
                QMessageBox.critical(
                    ...
                    "You can try manually in a terminal:\n"
    ...
    def _erstelle_ui(self) -> None:
        ...
        # Icon + Sprachname anzeigen.
        # Vorangestelltes Kürzel (z.B. 'DE - ' oder 'DE ') wird entfernt,
        # damit der Name nicht doppelt erscheint wenn die Flagge schon da ist.
        for code in lang_codes:
            image_code = mapping.get(code, code)
            raw_name = self.langs[code]["_meta"]["name"]
            # Erstes Wort entfernen – das ist immer das Emoji-Flag (🇩🇪, 🇺🇸 ...)
            # Beispiel: '🇩🇪 Deutsch' → 'Deutsch'
            #           '🇮🇱 עברית (Hebrew)' → 'עברית (Hebrew)'
            parts = raw_name.split(None, 1)   # maxsplit=1: erstes Wort abtrennen
            clean_name = parts[1].strip() if len(parts) > 1 else raw_name
        ...
        # Tooltip = voller Originalname beim Hover
        for i, code in enumerate(lang_codes):
            self.lang_combo.setItemData(i, self.langs[code]["_meta"]["name"], 3)
    ...
    def _open_language_tool(self):
        ...
        for code, name in zip(lang_codes, lang_names):
            # Erstes Wort (Emoji-Flag 🇩🇪) entfernen, Rest als Name anzeigen
            parts = name.split(None, 1)
            clean_name = parts[1].strip() if len(parts) > 1 else name
