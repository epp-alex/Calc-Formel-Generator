...
CALC_TR_FILE  = HERE / "language" / "libreoffice_calc_translations.json"


def _load_calc_translations() -> dict:
    """Load libreoffice_calc_translations.json.

    Structure: { "SUM": { "en": "SUM", "de": "SUMME", "fr": "SOMME", ... }, ... }
    Returns empty dict silently if the file is missing.
    Builds and caches an inverted lookup:
        _CALC_TR_BY_LANG["de"]["SUM"] = "SUMME"
    """
...
# Module-level cache: raw { "SUM": { "de": "SUMME", ... } }
_CALC_TR: dict = _load_calc_translations()


def _translate_formula(en_formula: str, target_lang: str) -> str:
    """Replace English function names in a formula with localized equivalents.

    Example: _translate_formula("=SUM(A1:A10)", "de") → "=SUMME(A1:A10)"
    If no translation exists for a function name, it is left unchanged.
    """
    if not _CALC_TR or not en_formula or target_lang == "en":
        return en_formula

    def _replace(match):
        en_name = match.group(1).upper()
        entry   = _CALC_TR.get(en_name, {})
        loc     = entry.get(target_lang, "")
        return (loc if loc else en_name) + "("

    # Match function names: word chars followed by "("
    return re.sub(r'([A-Za-z][A-Za-z0-9._]*)\(', _replace, en_formula)


def load_languages_from_json():
    """
    Loads languages and RTL flags from 'language/languages.json'.

    Expects a _meta block per language with at least:
        "_meta": { "name": "🇩🇪 Deutsch" }

    For RTL languages (Arabic, Hebrew, …) additionally:
        "_meta": { "name": "🇸🇦 العربية", "rtl": true }

    Returns: (langs, rtl_codes)
        langs     – sorted list of (code, display_name)
        rtl_codes – set() of all language codes where rtl=true is set
    """
    json_path = HERE / "language" / "languages.json"

    # Default fallback – no RTL
    default_langs    = [("de", "🇩🇪 Deutsch"), ("en", "🇺🇸 English")]
    default_rtl      = set()

    if not json_path.exists():
        print(f"Notice: {json_path} not found.")
        return default_langs, default_rtl
    ...
    try:
        ...
        for code, content in data.items():
            if isinstance(content, dict) and "_meta" in content:
                ...
                # Read RTL flag from the _meta block
                if meta.get("rtl", False):
                    rtl_codes.add(code)

        # Sort for a nicer UI list
        langs.sort(key=lambda x: x[1])

        return (langs if langs else default_langs), rtl_codes

    except Exception as e:
        print(f"Error parsing languages.json: {e}")
        return default_langs, default_rtl

# Automatic assignment – RTL_LANGUAGES comes directly from the JSON
ALL_LANGUAGES, RTL_LANGUAGES = load_languages_from_json()
...
def _apply_rtl(widget, lang_code: str):
    """
    Sets the text direction of an input field to RTL (right to left)
    for Arabic (ar) and Hebrew (he), otherwise LTR.
    Supports QLineEdit, QTextEdit and similar Qt widgets.

    Note: setAlignment is NOT set because Qt is in RightToLeft mode
    AlignRight internally mirrors (→ visually left), which would reverse the direction.
    setLayoutDirection alone is sufficient – Qt's Bidi algorithm
    handles the correct alignment automatically.
    """
...
class FormulaEditDialog(QDialog):
    ...
    def _copy_en_block(self):
        ...
        lines = [
            "Translate the following LibreOffice Calc formula entry into all 36 languages listed below.",
    ...
    def _paste_translations(self):
        ...
        for block in blocks:
            ...
            if not lines:
                continue
            code_raw = lines[0].strip()
            # Case-insensitive search, preserve original spelling (pt-BR)
            code = next((k for k in self._lang_fields if k.lower() == code_raw.lower()), None)
            if code is None:
                continue
    ...
    def get_data(self) -> dict:
        # formula: English only — translations handled at runtime via libreoffice_calc_translations.json
...
class PluginTranslationDialog(QDialog):
    ...
    def __init__(self, available_langs: list, parent=None,
        ...
        # RTL support: adjust text direction on start and on language change
        self._lang_combo.currentIndexChanged.connect(self._update_rtl_direction)
        self._update_rtl_direction()

    def _update_rtl_direction(self):
        """Adjusts the text direction of the input fields to match the selected language (RTL/LTR)."""
        code = self._lang_combo.currentData() or ""
        _apply_rtl(self._name, code)
        _apply_rtl(self._desc, code)
...
class FormulaTranslationDialog(QDialog):
    ...
    def __init__(self, plugin_dir: Path, parent=None):
        ...
        layout.addWidget(_hr())
        btn_row = QHBoxLayout()
        self._btn_copy_en      = QPushButton("📋  Copy all EN text")
        self._btn_paste        = QPushButton("📥  Insert translation")
        self._btn_missing_only = QPushButton("🤖  Only translate missing")
        self._btn_missing_only.setToolTip(
            "Creates an AI prompt ONLY for formulas that don't yet have a\n"
    ...
    def _load_language(self):
        ...
        if not lang:
            ...
            return
        self._lang_indicator.setText(f"Active: {lang_name}")

        for i, formula in enumerate(self._formulas):
            en_formula = _get_formula_display(formula.get("formula", ""))
            ...
            grp = QGroupBox(f"Formula {i+1}: {en_formula}")
            grp_layout = QFormLayout(grp)
            grp_layout.setSpacing(6)

            # English reference (read-only)
            ref = QLabel(f"EN formula: {en_formula}  │  Name: {en_name}  │  Category: {en_cat}")
            ref.setStyleSheet("color: #888; font-style: italic;")
            grp_layout.addRow("Reference (EN):", ref)

            # Translation fields (formula is auto-translated — no manual field needed)
    ...
    def _copy_en_texts(self):
        ...
        for i, formula in enumerate(self._formulas):
            ...
            lines.append(f"Formula: {en_formula}")
        ...
        QMessageBox.information(
            self, "Copied",
            f"{len(self._formulas)} Formula(s) copied to clipboard.\n\n"
            "Note: The 'Formula' field shows the English formula.\n"
            "Please replace with the formula of the target language.\n"
            "(e.g., =SUM → =SUMME for German, =SOMME for French).\n\n"
            "Then click '📥 Paste translation'."
        )
    ...
    def _copy_missing_only(self):
        ...
        if not missing_indices:
            QMessageBox.information(
                ...
                f"All formulas already have a translation for '{lang_name}'."
            )
            return

        lines = [
            f"Translate the following LibreOffice Calc formula entries into {lang_name} [{lang}].",
            "Output EXACTLY this format for each entry:",
            "",
            "---FORMULA <N>---",
            "Formula: <Formula in the target language — use correct function names>",
            "Name:",
            "Description:",
            "Category:",
            "",
            "Keep the formula numbers (---FORMULA N---) exactly as they are.",
            "Leave fields blank if there is no meaningful content.",
            "",
            "--- ENGLISH SOURCE TEXTS (missing only) ---",
        ]
        ...
        QMessageBox.information(
            self, "Prompt copied 📋",
            f"{len(missing_indices)} missing formula(s) for '{lang_name}' to the\n"
            "clipboard copied.\n\n"
            "Paste the text into an AI tool (Claude, ChatGPT…),\n"
            "copy the result and click '📥 Paste translation'."
        )
    ...
    def _paste_translations(self):
        ...
        """
        dlg = QDialog(self)
        dlg.setWindowTitle("Paste translation")
        dlg.setMinimumSize(560, 400)
        dlg_layout = QVBoxLayout(dlg)
        dlg_layout.setSpacing(8)
        dlg_layout.setContentsMargins(14, 10, 14, 10)

        dlg_layout.addWidget(_bold_label("Paste translated text", 10))
        dlg_layout.addWidget(_info_label(
            "Paste the translated text here (same structure as copying). "
        ...
        for num_str, content in zip(it, it):
            ...
            for line in content.strip().splitlines():
                ...
                val = val.strip()
                # Formula field
                if key_raw in ("formula", "formel", "формула", "formule",
                               "fórmula", "kaava", "τύπος", "képlet",
                               "الصيغة", "נוסחה"):
                    entry["formula"] = val
                # Name field — all supported languages
                elif key_raw in ("name", "ime", "имя", "ime", "nom", "naam",
                ...
                # Category field
                                 ...
                                 "カテゴリ", "카테고리"):
                    entry["category"] = val
                # Generic positional fallback: first unknown → name, then description, category
                else:
        ...
        if not parsed:
            QMessageBox.warning(self, "Error",
                ...
                "Please use the format from '📋 Copy all EN texts'.")
            return
        ...
        QMessageBox.information(self, "Pasted",
            f"{filled} Formula(s) populated.\n"
            "Please check the fields and then click '💾 Save Translations'.")
    ...
    def _save_translations(self):
        ...
        for i, fields in enumerate(self._fields):
            if i >= len(self._formulas):
                break
            # Name, Description, Category only — formula is auto-translated at runtime
...
# ---------------------------------------------------------------------------
# Missing Translations Dialog  –  "🔍 Check missing entries"
# ---------------------------------------------------------------------------
...
class MissingTranslationsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("🔍 Check for missing translations")
        ...
        layout.addWidget(_bold_label("Missing Translations Overview", 11))
        layout.addWidget(_info_label(
            "Shows all plugins and languages with missing translations."
            "Green = complete, Orange = missing."
        ))

        # Filter row
        filter_row = QHBoxLayout()
        filter_row.addWidget(QLabel("Filter by language:"))
        self._lang_filter = QComboBox()
        self._lang_filter.addItem("(All languages)", "")
        for code, name in ALL_LANGUAGES:
            if code != "en":
    ...
    def _refresh(self):
        ...
        for plugin_dir in plugins:
            ...
            if not plugin_has_issues and not filter_lang:
                item = QListWidgetItem(f"✅  {pname}  │  All languages complete")
                item.setForeground(QColor("#2e7d32"))
                self._result_list.addItem(item)

        if total_missing == 0:
            self._summary_lbl.setText(
                f"✅  No missing translations found  ({total_ok} Language/Plugin combinations checked)"
            )
            self._summary_lbl.setStyleSheet("font-weight: bold; color: #2e7d32;")
        else:
            self._summary_lbl.setText(
                f"⚠️  {total_missing} Missing formula translation(s) found"
            )
            self._summary_lbl.setStyleSheet("font-weight: bold; color: #B26A00;")


# ---------------------------------------------------------------------------
# Bulk Translate Dialog  –  "🌍 Translate all missing for one language"
# ---------------------------------------------------------------------------
...
class BulkTranslateDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("🌍 Bulk Translation – All Plugins")
        ...
        layout.addWidget(_bold_label("Bulk Translate for a Missing Language", 11))
        layout.addWidget(_info_label(
            "Select a language. The manager will then collect ALL missing formulas"
            "...from ALL plugins in a single AI prompt. "
            "After pasting, all plugins will be saved automatically."
        ))
        ...
        lang_row.addWidget(self._lang_combo)
        lang_row.addStretch()
        btn_scan = QPushButton("🔍  Scan for missing")
        ...
        # Scan result info
        self._info_lbl = QLabel("→ Click '🔍 Scan for missing' to start.")
        ...
        # Action buttons
        btn_row = QHBoxLayout()
        self._btn_copy  = QPushButton("📋  Copy AI prompt")
        self._btn_paste = QPushButton("📥  Paste & save translation")
        btn_close       = QPushButton("Close")
    ...
    def _scan(self):
        ...
        for plugin_dir in plugins:
            ...
            # Check plugin.json name and description
            plugin_name_missing = not (meta.get("name", {}) or {}).get(lang, "").strip()
            plugin_desc_missing = not (meta.get("description", {}) or {}).get(lang, "").strip()
            plugin_meta_missing = plugin_name_missing or plugin_desc_missing
            ...
            total_plugin_missing = len(missing) + (1 if plugin_meta_missing else 0)

            if missing or plugin_meta_missing:
                self._missing_map[plugin_dir] = missing
                total_missing += total_plugin_missing
                parts = []
                if plugin_meta_missing:
                    parts.append("plugin.json")
                if missing:
                    parts.append(str(len(missing)) + " formula(s)")
                label = "📦  " + pname + "  →  Missing:" + ", ".join(parts)
                item = QListWidgetItem(label)
                item.setForeground(QColor("#B26A00"))
                self._preview.addItem(item)
            else:
                item = QListWidgetItem("✅  " + pname + "  →  Complete")
                item.setForeground(QColor("#2e7d32"))
                self._preview.addItem(item)
        ...
        if total_missing:
            self._info_lbl.setText(
                f"⚠️  {total_missing} missing formula(s) for '{lang_name}' in "
                f"{len(self._missing_map)} plugin(s) found."
            )
            ...
            self._info_lbl.setText(
                f"✅  All formulas are already translated for '{lang_name}'!"
            )
    ...
    def _copy_prompt(self):
        ...
        lines = [
            f"Translate the following LibreOffice Calc formula entries into {lang_name} [{lang}].",
            "Output EXACTLY this format for each entry (keep numbers exactly as they are):",
            "",
            "---PLUGIN <plugin_id> META---         ← Plugin name and description",
            "Name:",
            "Description:",
            "",
            "---PLUGIN <plugin_id> FORMULA <N>---  ← Formula entries",
            "Name:",
            "Description:",
            "Category:",
            "",
        ...
        for plugin_dir, missing_list in self._missing_map.items():
            ...
            # plugin.json name/description if missing
            meta = _load_json(plugin_dir / "plugin.json") or {}
            pn_en = (meta.get("name", {}) or {}).get("en", "")
            pd_en = (meta.get("description", {}) or {}).get("en", "")
            pn_tl = (meta.get("name", {}) or {}).get(lang, "").strip()
            pd_tl = (meta.get("description", {}) or {}).get(lang, "").strip()
            if not pn_tl or not pd_tl:
                lines.append(f"---PLUGIN {plugin_dir.name} META---")
                lines.append(f"Name: {pn_en}")
                lines.append(f"Description: {pd_en}")
        ...
        QMessageBox.information(
            ...
            "1. Paste it into an AI tool (Claude, ChatGPT…)\n"
            "2. Copy the result\n"
            "3. Click '📥 Paste & save translation' here'"
        )
    ...
    def _paste_and_save(self):
        ...
        dlg_l.addWidget(_bold_label("Paste AI Translation", 10))
        dlg_l.addWidget(_info_label(
            "Paste the AI tool's response here.\n"
        ...
        # Parse: ---PLUGIN <id> META--- und ---PLUGIN <id> FORMULA <N>---
        token_re = re.compile(
            r'-{3}PLUGIN\s+(\S+)\s+(META|FORMULA\s+\d+)-{3}', re.IGNORECASE
        )
        parsed_formulas: Dict[str, Dict[int, dict]] = {}
        parsed_meta:     Dict[str, dict]             = {}

        parts = token_re.split(raw)
        it = iter(parts)
        next(it)
        for pid, tag, block in zip(it, it, it):
            entry: dict = {}
            ...
            if tag_clean == "META":
                parsed_meta[pid] = entry
            elif tag_clean.startswith("FORMULA"):
                idx = int(tag_clean.split()[-1]) - 1
                parsed_formulas.setdefault(pid, {})[idx] = entry
        ...
        for plugin_dir, formulas in self._plugin_formulas.items():
            ...
            # Save plugin.json name and description
            if pid in parsed_meta:
                meta = _load_json(plugin_dir / "plugin.json") or {}
                meta_entry = parsed_meta[pid]
                for key in ("name", "description"):
                    val = meta_entry.get(key, "").strip()
                    if val:
                        if not isinstance(meta.get(key), dict):
                            meta[key] = {"en": meta.get(key, "") if isinstance(meta.get(key), str) else ""}
                        meta[key][lang] = val
                        saved_entries += 1
                _save_json(plugin_dir / "plugin.json", meta)
                changed = True

            # Save formulas.json entries
            if pid in parsed_formulas:
                for idx, entry in parsed_formulas[pid].items():
                    if idx >= len(formulas):
                        continue
                    for key in ("name", "description", "category"):
                        ...
                        if val:
                            ...
                            changed = True
                            saved_entries += 1
            if changed and pid in parsed_formulas:
                _save_json(plugin_dir / "formulas.json", formulas)
                saved_plugins += 1

        QMessageBox.information(
            self, "Saved ✅",
            f"{saved_entries} Entries saved in {saved_plugins} plugin(s).\n\n"
            "Click '🔍 Scan missing' to update the status."
        )
        self._scan()
...
class PluginManagerWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # ── Scan button: missing translations ──────────────────────────────
...
class ImportantNoticeDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # PATH TO FILE: we look for the file in the same folder as the script
        ...
        if notice_path.exists():
            try:
                ...
                with open(notice_path, "r", encoding="utf-8") as f:
                    content = f.read()
                text.setPlainText(content) # Displays the text of your .md file
            except Exception as e:
                text.setHtml(f"<b style='color:red;'>Error loading file:</b><br>{str(e)}")
        else:
            text.setHtml("<b style='color:red;'>Error:</b> The file <code>IMPORTANT_NOTICE.md</code> was not found.")
